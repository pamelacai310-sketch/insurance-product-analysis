...
description: 在相同投保条件下比较多只保险产品的现金价值、累计保费回收率、保证退保 IRR、身故保险金、年金领取效率、长寿风险转移、合同选择权和保证利益压力情景。用户提供保险费率表、现金价值表、保险条款、产品说明书、投保计划或结构化数据，询问哪个产品现金价值或身故保险金更高、何时回本、IRR 排名、同样保费如何换算基本保额，或要求核查千元口径和表格版本时使用。适用于 compare insurance products、cash value、death benefit、surrender IRR、premium rate table 等英文请求。
---
...
- 只可写明具体保单年度、利益口径和指标集合下的“窗口领先”；禁止把单一节点或有限指标外推为“综合领先”或全周期冠军。
- 年金效率只从逐年保证领取表计算；没有正式给付表时标记缺数据，不用基本保额或产品名称推断。
- 长寿尾部价值只累计明确覆盖测试年龄的保证领取；“终身领取”不自动换算为高分。
- 合同选择权没有显式增量现金流时只作条款审计，不把保单贷款、减保等功能存在性计分。
- 压力测试使用保证现金价值的零分红提前退出情景，不使用随机ALM或VaR结果评价单一产品。

## 输出要求

同时交付：

- `comparison.md`：结论、统一假设、证据与单位审计、现金价值和回收率、保证退保 IRR、身故保险金和杠杆、年金效率、长寿风险转移、合同选择权、保证压力测试、红利口径及缺失数据。
