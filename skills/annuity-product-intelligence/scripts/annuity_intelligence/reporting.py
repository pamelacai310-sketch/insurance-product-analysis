    ...
    for config in normalized["configurations"]:
        ...
        for scenario_id, scenario in item["scenarios"].items():
            ...
            if curve:
                lines.extend(
                    [
                        "| 保单月 | 现金价值 | CV/累计保费 | 锁定比例 | 现金价值IRR | 含既往领取退出IRR |",
                        "|---:|---:|---:|---:|---:|---:|",
                    ]
                )
                for row in curve:
                    lines.append(
                        f"| {row['policy_month']} | {_money(row['cash_value'], currency)} | "
                        f"{_percent(row['cash_value_ratio'])} | {_percent(row['lock_ratio'])} | "
            ...
            lines.extend(
                [
                    ...
                    "",
                    "| 生存年龄 | 累计年金 | 领取倍数 | 仅领取IRR | 含剩余现金价值IRR |",
                    "|---:|---:|---:|---:|---:|",
                ]
            )
            ...
            for row in survival_curve:
                lines.append(
                    f"| {row['target_age']} | {_money(row['cumulative_annuity'], currency)} | "
                    f"{_number(row['payout_multiple'])} | {_irr(row['income_only_irr'])} | "
            ...
            lines.extend(
                [
                    ...
                    "",
                    "| 生存年龄 | 假设通胀率 | 实际累计年金 | 当年实际年化领取 | 实际领取倍数 | 实际仅领取IRR |",
                    "|---:|---:|---:|---:|---:|---:|",
                ]
            )
            ...
            if death_curve:
                lines.extend(
                    [
                        "| 身故年龄 | 已交保费 | 身故结算 | 受益人后续领取 | 名义回收率 | 条件身故结果IRR |",
                        "|---:|---:|---:|---:|---:|---:|",
                    ]
                )
                for row in death_curve:
                    if row.get("status") != "available":
                        ...
                        lines.append(
                            f"| {row['target_age']} | {_money(row.get('cumulative_premium_paid', {}), currency)} | "
                            f"{_money(row.get('death_settlement', {}), currency)} ({status}) | "
                            f"{_money(row.get('beneficiary_continuation', {}), currency)} | — | — |"
                        )
                        continue
                    lines.append(
                        f"| {row['target_age']} | {_money(row['cumulative_premium_paid'], currency)} | "
                        f"{_money(row['death_settlement'], currency)} | {_money(row['beneficiary_continuation'], currency)} | "
